...
@section('content')
    <div class="panel panel-default">
        <div class="panel-body">
            <form method="post" action="{{ route('backend.groups.store')}}" class="form-horizontal form-group" data-parsley-validate novalidate>
                <div class="form-group {{ formHasError('name') }}">
                    <label class="col-sm-2 control-label" for="in-name">{{ trans('form.groups.name') }}</label>
                    <div class="col-sm-10">
                        <input type="text" name="name" id="in-name" required maxlength="255" class="form-control">
                        {!! formAlertError('name') !!}
                    </div>
                </div>
                                ...
                                @foreach($val['permissions'] as $k =>$item)
                                    <div class="col-sm-2">
                                        ...
                                        <label class="radio c-radio allow" for="{{ $key . $k }}-allow">
                                            <input type="radio" id="{{ $key . $k }}-allow" name="permissions[{{ $k }}]" value="1">
                                            <span class="fa fa-check"></span>{{ $allow }}
                                        </label>
                                        <label class="radio c-radio deny" for="{{ $key . $k }}-deny">
                                            <input type="radio" id="{{ $key . $k }}-deny" name="permissions[{{ $k }}]" value="0" checked>
