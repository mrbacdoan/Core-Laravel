    ...
    <div class="col-md-12">
        <div class="box box-danger">
            <div class="box-header">
                <h3 class="box-title">{{ trans('backend/title.categories.edit', ['name' => $category->name]) }}</h3>
            </div>
            {!! Form::model($category, array('route' => array('backend.categories.update', $category->id), 'method' => 'PUT', 'class' => 'form-horizontal'))  !!}
                <div class="box-body">
                    ...
                    <div class="form-group {{formHasError('parent_id')}}">
                        ...
                        <div class="col-sm-9">
                            <select class="form-control" name="parent_id">
                                ...
                                {!! recursionSelected($categories, $parent = 0, $txt = '', $category->id, $category->parent_id) !!}
                            </select>
                    ...
                    <div class="form-group {{formHasError('status')}}">
                        ...
                        <div class="col-sm-9">
                            <label>{!! Form::radio('status', STATUS_ACTIVATED, ($category->status == STATUS_ACTIVATED) ? true : false) !!}{{trans('backend/title.active') }}</label>
                            <label>{!! Form::radio('status', STATUS_DEACTIVATED, ($category->status == STATUS_DEACTIVATED) ? true : false)!!}{{trans('backend/title.deactive') }}</label>
                        </div>
